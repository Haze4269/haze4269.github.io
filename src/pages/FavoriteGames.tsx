          ...
          <div className="bg-card rounded-lg retro-dashed p-3 sm:p-4 space-y-3">
            <div className="flex items-center justify-between border-b-2 border-hell-red pb-2 gap-2">
              <div className="flex items-center gap-1 md:gap-2 min-w-0 flex-1">
                <Gamepad2 className="w-4 h-4 md:w-5 md:h-5 text-hell-red animate-pulse flex-shrink-0" />
                ...
                {gamesData.games.map((game, index) => (
                  <div key={index} className="bg-card rounded-lg retro-dashed p-3 md:p-4 space-y-2 border-2 border-hell-red/30 hover:border-hell-red transition-colors">
                    <div className="flex items-center gap-2">
                      ...
                      <h3 className="font-bold uppercase text-hell-red tracking-wide text-sm md:text-base">{game.name}</h3>
                    </div>
                    <p className="text-xs text-muted-foreground font-semibold uppercase">{game.platform}</p>
                    <p className="text-xs md:text-sm text-muted-foreground leading-relaxed font-semibold">{game.description}</p>
                  </div>
                ))}
              </div>
            ) : (
              <div className="text-center py-8 text-muted-foreground">Failed to load games</div>
            )}
          </div>
        </div>
      </div>
